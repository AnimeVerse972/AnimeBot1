...
from database import (
    ...
    get_today_users,
    add_anime,
    add_part_to_anime,search_anime_by_title,
    ...
    if user_id in ADMINS:
        ...
        if not BOT_ACTIVE:
            await message.answer("📴 Bot hozircha o'chirilgan.")
       	    ...
       	    kb = ReplyKeyboardMarkup(resize_keyboard=True)
       	    kb.add(KeyboardButton("🔍 Anime qidirish"))  # 🔝 Tepada qidirish tugmasi
       	    kb.add(
...
async def show_bot_status(message: types.Message):
    ...
    if BOT_ACTIVE:
        inline_kb.add(InlineKeyboardButton("🔴 O'chirish", callback_data="bot_toggle:off"))
    else:
        inline_kb.add(InlineKeyboardButton("🟢 Yoqish", callback_data="bot_toggle:on"))
    inline_kb.add(InlineKeyboardButton("⬅️ Orqaga", callback_data="bot_status_back"))

    await message.answer(text, reply_markup=inline_kb, parse_mode="HTML")

@dp.callback_query_handler(lambda c: c.data.startswith("bot_toggle:"))
...
@dp.callback_query_handler(lambda c: c.data == "bot_status_back")
async def back_from_bot_status(callback: types.CallbackQuery):
...
# === Qidiruv natijasini inline tugmalar bilan ko'rsatish ===
...
async def handle_search(message: types.Message, state: FSMContext):
    ...
    await message.answer("🔎 *Topilgan animelar:*", reply_markup=markup, parse_mode="Markdown")

# === Callback: anime ko'rsatish ===
@dp.callback_query_handler(lambda c: c.data.startswith("show_anime:"))
async def show_full_anime(callback: types.CallbackQuery):
    ...
    # Reklama postini yuborish (rasm + caption + "Tomosha qilish" tugmasi)
    await send_reklama_post(user_id, code)
...
async def start_posting(message: types.Message, state: FSMContext):
    ...
    await message.answer("📌 Qaysi animeni post qilmoqchisiz? Kodni yuboring:", reply_markup=control_keyboard())
...
async def handle_code_message(message: types.Message):
    ...
    # Qolgan qism — obuna, stat, post yuborish...
    unsubscribed = await get_unsubscribed_channels(message.from_user.id)
    if unsubscribed:
        markup = await make_unsubscribed_markup(message.from_user.id, message.text)
        await message.answer(
            "❗ Anime olishdan oldin quyidagi kanal(lar)ga obuna bo‘ling:",
            reply_markup=markup
        )
